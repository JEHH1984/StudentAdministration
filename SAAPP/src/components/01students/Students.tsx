...
export function Students() {
    ...
    const doPut = (bodyData: any) => {
        fetch(`${BASE_URL}/${bodyData.id}`, {
            ...
            .then((response) => response.json())
            .then((data) => {
                const newStudents = students.filter((student: IStudent) => student.id !== data.id);
    ...
    const handleSubmitFilter = (e: any) => {
        ...
        const filteredStudents = students.filter((student: IStudent) => (
            student.nombre.toLowerCase().includes(search) || student.apellido.toLowerCase().includes(search)
        ))
            ...
            <section className="studentSection">
                {showNew ? (
                    <form onSubmit={handleSubmit}>
                        <input type="text" placeholder="Nombre" name="nombre" defaultValue={selectedStudent?.nombre} />
                        <input type="text" placeholder="Apellido" name="apellido" defaultValue={selectedStudent?.apellido} />
                        <div>
                            <button type="submit">{selectedStudent ? "Actualizar" : "Guardar"}</button>
                ...
                {filteredStudents.map((student: IStudent) => (
                    <div key={student.id}>
                        ...
                        <div style={{ display: "flex", gap: "10px" }}>
                            ...
                            <button onClick={() => handleDelete(student.id)}>Editar</button>
                        </div>
                    </div>
                ))}
            </section>
